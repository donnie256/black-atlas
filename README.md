...
- Server-side validation, honeypot protection, Turnstile verification, and rate limiting
...
Create `.env.local` from `.env.example` and fill in Supabase credentials. Turnstile is optional in local development when both Turnstile env vars are omitted.
